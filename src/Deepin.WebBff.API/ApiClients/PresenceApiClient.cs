...
: HttpClientBase(logger, httpClient), IPresenceApiClient
{
    ...
    protected override string BaseUrl => _urlsConfig.PresenceUrl;
    public async Task<UserPresenceDto> GetPresenceAsync(string userId)
    {
        var request = new HttpRequestMessage(HttpMethod.Get, $"api/v1/presence/{userId}");
    ...
    {
        var request = new HttpRequestMessage(HttpMethod.Get, $"api/v1/presence/batch?ids={string.Join(",", userIds)}");
